...
function SKLogo() {
  return (
    <Link to="/" style={{textDecoration:"none", display:"flex", alignItems:"center", gap:10, perspective: 1000}}>
      <motion.div
        animate={{ rotate: 360 }}
        transition={{ duration: 15, repeat: Infinity, ease: "linear" }}
        whileHover={{ scale: 1.2, filter: "drop-shadow(0 0 25px rgba(90,200,250,0.8))" }}
        style={{ transformStyle: "preserve-3d", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}
      >
        <svg width="40" height="40" viewBox="0 0 64 64" style={{ overflow: "visible" }}>
          ...
          <motion.path
            ...
            strokeDasharray="4 4" filter="url(#goldGlow)"
          />
          <path
            d="M32 2 L38 12 L50 14 L48 26 L56 34 L48 42 L50 54 L38 52 L32 62 L26 52 L14 54 L16 42 L8 34 L16 26 L14 14 L26 12 Z"
          ...
          />
          <motion.polygon
            animate={{ rotate: -720 }} transition={{ duration: 30, repeat: Infinity, ease: "linear" }}
            style={{ transformOrigin: "32px 32px" }} points="32,10 54,48 10,48"
            fill="none" stroke="url(#contractorGrad)" strokeWidth="2"
          />
          <motion.polygon
            animate={{ rotate: 720 }} transition={{ duration: 30, repeat: Infinity, ease: "linear" }}
            style={{ transformOrigin: "32px 32px" }} points="32,54 10,16 54,16"
      ...
      <div style={{ display: 'flex', flexDirection: 'column' }}>
        <div style={{fontFamily:"'Inter', sans-serif", fontWeight:800, fontSize:16, color: "#FFFFFF", lineHeight:1.1, letterSpacing: -0.5}}>
          SK <span style={{
        ...
        <div style={{fontSize:9, color: "#94A3B8", letterSpacing:2, textTransform:"uppercase", fontWeight: 700}}>Nashik • Premium</div>
      </div>
    </Link>
  );
}
...
function Navbar() {
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'center', position: 'fixed', top: 12, left: 0, right: 0, zIndex: 100 }}>
      <motion.nav
        initial={{ y: -100 }}
        animate={{ y: 0 }}
        ...
        style={{
          ...
          background: sc ? "rgba(15, 23, 42, 0.97)" : "rgba(30, 41, 59, 0.85)",
      ...
      >
        {/* Top bar */}
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0.55rem 1.25rem" }}>
          <SKLogo />
                  ...
                  {loc.pathname === n.path && (
                    <motion.div layoutId="nav-indicator"
                      style={{
                        ...
                        background: clr, borderRadius: 2, boxShadow: `0 0 8px ${clr}`
                      }}
                    />
                  )}
                </Link>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <Link to="/contact" className="desktop-nav">
              <motion.div
                ...
                style={{
                  ...
                  color: "#FFFFFF", borderRadius: 1e5, padding: "10px 20px", fontSize: 11,
                  letterSpacing: 1.5, textTransform: "uppercase", fontWeight: 800,
                  boxShadow: "0px 4px 10px rgba(0,0,0,0.3)", whiteSpace: "nowrap"
                }}
              >
                Contact Us
              </motion.div>
            ...
            >
              {[0,1,2].map(i => (
                <motion.span key={i}
                  animate={open ? (i===0 ? {rotate:45,y:8} : i===2 ? {rotate:-45,y:-8} : {opacity:0}) : {rotate:0,y:0,opacity:1}}
                  style={{ display:"block", width:20, height:2, background:"#fff", borderRadius:2 }}
                />
              ))}
            </button>
          </div>
                  ...
                  return (
                    <Link key={n.path} to={n.path}
                      style={{
                        ...
                        borderLeft: active ? `3px solid ${clr}` : "3px solid transparent",
                        fontSize: 15, fontWeight: 700, color: active ? clr : "#CBD5E1",
                        letterSpacing: 0.5, textDecoration: "none", transition: "all 0.2s"
                      }}
                    >
                      {n.label}
                    </Link>
      ...
      {/* Hide/show helpers via inline style injection */}
      <style>{`
...
function FloatingButtons() {
  return (
    <div style={{position:"fixed",bottom:24,left:16,zIndex:50,display:"flex",flexDirection:"column",gap:10}}>
      <a href="tel:8619181791" style={{width:48,height:48,background:"#1A1510",color:"#fff",borderRadius:"50%",
        display:"flex",alignItems:"center",justifyContent:"center",
        boxShadow:"0 4px 16px rgba(0,0,0,0.3)",textDecoration:"none",fontSize:18,
      ...
      <a href="https://wa.me/918619181791?text=Hi,%20I%20need%20marble/tile%20work" target="_blank" rel="noopener noreferrer"
        style={{width:48,height:48,background:"#25D366",color:"#fff",borderRadius:"50%",
          display:"flex",alignItems:"center",justifyContent:"center",
          boxShadow:"0 4px 16px rgba(37,211,102,0.4)",textDecoration:"none",fontSize:18}}>💬</a>
    </div>
  );
}
  ...
  return (
    <footer style={{background:"#1A1208",borderTop:"1px solid rgba(196,152,42,0.2)"}}>
      <div className="footer-grid" style={{
        maxWidth:1280, margin:"0 auto", padding:"3rem 1.5rem 1.5rem",
        display:"grid", gridTemplateColumns:"2fr 1fr 1fr 1.5fr", gap:32
      }}>
        <div>
          <SKLogo/>
        ...
        <div>
          ...
          {[["📞","8619181791 / 9028933305"],["📧","SkTiles&marbles@gmail.com"],["📍","Takli Road, Taphovan, Nashik, MH"],["⏰","Mon–Sat: 9AM – 7PM"]].map(([ic,v],i)=>(
            <div key={i} style={{display:"flex",gap:8,marginBottom:10,color:"#6B5F45",fontSize:13}}>
              <span>{ic}</span><span style={{wordBreak:"break-word"}}>{v}</span>
            </div>
          ))}
        </div>
      </div>
      <div style={{maxWidth:1280,margin:"0 auto",padding:"1rem 1.5rem",borderTop:"1px solid rgba(196,152,42,0.12)",display:"flex",justifyContent:"space-between",flexWrap:"wrap",gap:8}}>
